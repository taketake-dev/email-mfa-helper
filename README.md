...
送信ボタンは自動で押さない。認証コードはMoodleの入力欄への設定以外で表示せず、メール情報、認証用リンク、資格情報を保存・外部送信しない。初回は、Outlook Webの受信トレイ一覧を一時確認することへの明示同意が必要である。
...
- 同意は拡張機能の設定画面から取り消せる。取り消すと、実行中の自動確認も停止する。
